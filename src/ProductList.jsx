   ...
   const handleContinueShopping = (e) => {
    ...
    return (
        ...
        <div className="product-grid">
            {plantsArray.map((plantCat, ind) => (
                <>
                <h1>{plantCat.category}</h1>
                <div className="product-list" key={ind}>
    
                {
                    plantCat.plants.map((plant, index) => 
                        (<div className="product-card" key={index}>
                            <h3 className='product-title'>{plant.name}</h3>
                            <img src={plant.image} alt={`${plant.name}-image`} className='product-image'/>
                            <h3>{plant.cost}</h3>
                            <p>{plant.description}</p>
                            <button className='product-button' onClick={(e) => handleAddToCart(plant, e)} disabled={addedToCart[plant.name]}>Add to Cart</button>
                        </div>)
                    )
                }
            </div>
